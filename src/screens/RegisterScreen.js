...
async function uriToJpegBlob(uri) {
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      ...
      // Always draw to a white background to remove alpha channel
      ctx.fillStyle = '#fff';
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.drawImage(img, 0, 0);
      // Force JPEG output (no alpha)
      canvas.toBlob(
        (blob) => {
...
async function processImageForUpload(image, index) {
  if (Platform.OS === 'web') {
    // For web: convert to JPEG blob
    if (image.uri && image.uri.startsWith('data:')) {
      // Base64 data URI
      const blob = await base64ToJpegBlob(image.uri);
      return new File([blob], `employee_photo${index+1}.jpg`, { type: 'image/jpeg' });
    } else if (image.uri) {
      // Regular URI
    ...
    // For native: use the image as is, but ensure proper format
    ...
    if (imageUri.startsWith('data:')) {
      ...
      return {
        uri: imageUri,
        type: 'image/jpeg',
        name: `employee_photo${index+1}.jpg`,
      };
    }
  }
}
...
const RegisterScreen = ({ navigation }) => {
  ...
  const registerEmployee = async () => {
    ...
    try {
      ...
      // Debug: log FormData keys and types
      for (let pair of formData.entries()) {
        console.log(pair[0], pair[1]);
      }
      
      const response = await fetch('http://192.168.29.195:5000/register', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        Alert.alert('Registration Failed', result.error || 'Failed to register employee. Please try again.');
      }
    } catch (error) {
    ...
    return (
      <View style={styles.container}>
        <Card style={styles.card}>
          <Card.Content>
            <Text style={styles.errorText}>Camera access is required for registration</Text>
      ...
      <Card style={styles.card}>
        <Card.Content>
          ...
          <Text style={styles.subtitle}>
            Capture 1-3 clear, front-facing photos for better face recognition accuracy
          </Text>
      ...
      <Card style={styles.card}>
        <Card.Content>
          <Button
            ...
            disabled={!employeeName.trim() || !employeeId.trim() || capturedImages.length === 0 || isRegistering}
